...
and the assumption was made that the wrench $\vec{w}_{F}$ (see eq. \ref{eq:joint_space_dyn})
exerted by the end effector on the environment
...
\begin{equation}
  \prescript{ws}{}{ \vec{f}}_{s} = -\prescript{ws}{}{ \vec{f}}_{pl} -\prescript{ws}{}{ \vec{f}}_{c} -m \prescript{ws}{}{ \vec{g}}
  + m \prescript{ws}{}{ \vec{a}}_{cm}
\end{equation}
\begin{equation}\label{eq:ne_torque_1}
  \begin{split}
  \prescript{ws}{}{ \vec{\tau}}_{s} =& -\prescript{ws}{}{ \vec{\tau}}_{pl} -\prescript{ws}{}{ \vec{\tau}}_{c}
  - \prescript{ws}{}{ \tilde{\vec{p}}}_{c} \prescript{ws}{}{ \vec{f}}_{c}
  + \prescript{ws}{}{ \tilde{\vec{g}}} m\prescript{ws}{}{\vec{c}} \\
  & - \prescript{ws}{}{ \tilde{\vec{a}}}_{cm} m \prescript{ws}{}{ \vec{c}}
  + \prescript{ws}{}{ I}_{cm} \prescript{ws}{}{\vec{\alpha}}
  + \prescript{ws}{}{ \tilde{\vec{\omega}}} \prescript{ws}{}{ I}_{cm} \prescript{ws}{}{ \vec{\omega}}\\
  \end{split}
\end{equation}
where
...
\par
It should be noted that the wrench $\vec{w}_{F}$  is given by
$\begin{bmatrix}
 -\vec{f}_{c} ^T, & - (\vec{\tau}_{c} +\tilde{\vec{p}}_{c} \vec{f}_{c})^T
\end{bmatrix}^T$
...
\[
\vec{f}_{sw,off} = \vec{f}_{s,0} = -\vec{\tau}_{pl} -m \prescript{ws}{}{ \vec{g}}_{0}
\]
\[
\vec{\tau}_{sw,off} = \vec{\tau}_{s,0} = -\vec{\tau}_{pl} + \prescript{ws}{}{ \tilde{\vec{g}}}_{0} m\vec{c}
\]
...
\begin{equation}
  \label{eq:m_force_2}
  \vec{f}_{m} = \vec{f}_{c} +m \vec{g} -m \prescript{ws}{}{ \vec{g}}_{0} - m  \vec{a}_{cm}
\end{equation}
\begin{equation}
  \label{eq:m_torque_2}
  \vec{\tau}_{m} = \vec{\tau}_{c} + \tilde{\vec{p}}_{c}  \vec{f}_{c}
  - \tilde{\vec{g}} m\vec{c} + \prescript{ws}{}{ \tilde{\vec{g}}}_{0} m\vec{c} +  \tilde{\vec{a}}_{cm} m  \vec{c}
\end{equation}
...
mass with respect to the wrist $\vec{c}$ and the offsets $-m \prescript{ws}{}{ \vec{g}}_{0}$
and $\prescript{ws}{}{ \tilde{\vec{g}}}_{0} m\vec{c}$.
The gravity $\prescript{ws}{}{ \vec{g}}$ expressed in sensor frame can be obtained using the direct kinematics software facilities
...
\begin{split}
  ...
  \end{bmatrix} &=
  \begin{bmatrix}
    \vec{g} m + (-m \prescript{ws}{}{ \vec{g}}_{0}) \\
    -\tilde{\vec{g}} m\vec{c} + (\prescript{ws}{}{ \tilde{\vec{g}}}_{0} m\vec{c}) \\
  \end{bmatrix}\\
  & =
  \begin{bmatrix}
  ...
  \begin{bmatrix}
    m \\
    m \vec{c} \\
    -m \prescript{ws}{}{ \vec{g}}_{0} \\
    \prescript{ws}{}{ \tilde{\vec{g}}}_{0} m\vec{c} \\
  \end{bmatrix}\\
  &=
  H(\prescript{ws}{}{ \vec{g}}) \vec{\theta}
\end{split}
\]
where $\prescript{ws}{}{ \vec{g}} = \prescript{ws}{}{ \vec{g(\vec{q})}}$ depends on the configuration $\vec{q}$ of the robot.
\par
...
\item $-\hat{m} \prescript{ws}{}{ \hat{\vec{g}}}_{0} =
  \begin{bmatrix}
    0.8577 & -6.0316 & 0.8438 
  \end{bmatrix}^T
  $N  (see Fig. \ref{fig:off_f_estimation})
\item $\prescript{ws}{}{ \tilde{\hat{\vec{g}}}}_{0} \hat{m}\hat{\vec{c}} =
  \begin{bmatrix}
